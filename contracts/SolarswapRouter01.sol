...
contract SolarswapRouter01 is ISolarswapRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISolarswapPair(SolarswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SolarswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
